...
API_FORM = {
    "content": {
        "survey": [
            ...
            {"type": "date", "name": "S0_Q1", "label": ["0.1 ວັນທີໃຫ້ສໍາພາດ"]},
            {"type": "select_one", "select_from_list_name": "learn",
             "name": "S4_Q1", "label": ["4.1 ເຄີຍຮັບຮູ້ບໍ"]},
            {"type": "end_group"},
        ],
        "choices": [
...
API_SUBS = [
    {"Section_2/S2_Q1": "restaurant", "S2_Q8": "1 2", "S0_Q1": "2026-06-30", "S4_Q1": "1",
     "_submission_time": "2026-06-30T03:00:00", "_id": 1},
    {"Section_2/S2_Q1": "hotel", "S2_Q8": "1", "S0_Q1": "2026-06-29", "S4_Q1": "0",
...
def test_interview_date_decoded():
    df = decode.decode_submissions(API_SUBS, API_FORM)
    # S0_Q1 (date type) -> formatted interview-date label
    assert list(df["_idate_label"]) == ["30/06/2026", "29/06/2026"]
...
def test_build_question_labels_ordered():
    ...
    assert list(labels.keys()) == ["S2_Q1", "S2_Q8", "S0_Q1", "S4_Q1"]  # survey order
